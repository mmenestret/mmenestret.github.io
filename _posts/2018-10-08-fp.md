...
- _Totality_: _pure functions_ from type `A => B` (`A` are called the _domain_ and `B` the _co-domain_), __have to be defined for every value of its domain__, here for every value of type `A`
