...
## Data / behavior relation

## Expression problem

Comment se comporte une code base existante quand on ajoute à un programme:

- De nouveaux cas à un type existant
- De nouveaux comportements sur un type existant
...
- 👍 : Ajout de nouveaux cas aux types
    - Nouvelle _classe_ + _extends_ (l'existant reste inchangé)
- 👎 : Ajout d'un nouveau comportement
    - Nouvelle fonction sur l'interface (impact sur tous les sous types existant...)
...
```scala
trait MonType { def comportement: String }

case class A() extends MonType { override def comportement: String = "I'm A" }
case class B() extends MonType { override def comportement: String = "I'm B" }
```

---

👍 : Ajout d'un nouveau cas à mon type

```scala
case class C() extends MonType { override def comportement: String = "I'm C" }
```
...
```scala
trait MonType { 
    def comportement: String
    def nouveauComportement: String
}
```

Il faut repasser sur toutes les _classes_ qui extends `MonType` pour l'implémenter !
...
- 👍 : Ajout d'un nouveau comportement
    - Nouvelle fonction qui traite tous les cas de mon type (l'existant reste inchangé)
- 👎  : Ajout de nouveaux cas aux types
    - Impact sur toutes les fonctions existantes traitant ce type pour traiter ce nouveau cas...
...
```scala
sealed trait MonType
final case class A() extends MonType
final case class B() extends MonType

def comportement(m: MonType): String = m match {
    case A() ⇒ "I'm A"
    case B() ⇒ "I'm B"
}
```
...
```scala
def nouveauComportement(m: MonType): String = m match {
    case A() ⇒ ???
    case B() ⇒ ???
  }
```
...
```scala
final case class C() extends MonType
```

Il faut repasser sur toutes les fonction pour implémenter le nouveau cas !
