...
But functors have to respect some rules, called _functor_'s laws...
...
```scala
val halve: Int => Float = _.toFloat / 2
```

Here we defined the function from `Int` to `Float` that we are going to map over our containers
...
We can see that an `Option[Int]` turns into an `Option[Float]`, the inner value of the container is modified when mapped over with a function from `Int` to `Float`... 
...
We can see that an `List[Int]` turns into a `List[Float]`, the inner values of the container are modified when mapped over with a function from `Int` to `Float`... 
...
We can see that an `UselessContainer[Int]` turns into an `UselessContainer[Float]`, the inner value of the container is modified when mapped over with a function from `Int` to `Float`... (I deliberately hide an implementation detail here, I'll cover it later)
...
_Functor_ is usually represented by a _type class_.

As a reminder, a _type class_ is a group of types that all provide the same abilities, which make them part of the same class (group) of (same abilities providing) types (see [here]({{ site.baseurl }}{% post_url 2018-10-05-typeclasses %})).

Here the types of our _functor_ type class are type constructors, `F[_]`, and the type class expose a `map` function that takes a container of type `F[A]`, a function `A => B` that will return a `F[B]`.
...
That way, for a container `F[_]` to be a _functor_, it must be an instance of the _functor_ type class, hence, provide an instance of `Functor`.
...
- For all values `uc` of type `UselessContainer` (pseudo code): 
    ```scala
    ucFunctor.map(uc, identity) == uc
    ```
- For all values `uc` of type `UselessContainer` and for any two functions `f` and `g` (pseudo code):
    ```scala
...
During this article, we only talked about the most widely used kind of _functors_, the _co-variant endofunctors_. Don't mind the complicated name, they are all you need to know to begin having fun in functional programming.

However if you'd like to have a grasp about the theory behind _functors_, keep on reading.

_Functors_ are mappings between categories.
...
- And two fundamental properties:
    ...
    - __Identity__: There is an identity arrow for every object in the category which is the arrow which goes from an object to itself
...
In the context of pure functional programming in _Scala_, we can consider that we work in a particular category that we are going to call it `S` (I won't go into theoretical compromises implied by that parallel, but there are some theoretical comprises !):

- `S` objects are _Scala_'s types
- `S` morphisms are _Scala_`s functions
    - Composition between morphisms is then function composition
    - Identity morphisms for `S` objects is the identity function
    
Indeed, if we consider that _Scala_'s types are our objects, to go from one object `a` (the type `A`) to an other object `b` (the type `B`), _Scala_ functions `A => B` are morphisms between `a` and `b`.

Given our morphism from `a` to `b`, if it exists an object `c` (the type `C`) and a morphism between `b` and `c` exists (a function `B => C`):

- Then it must exists a morphism from `a` to `c` which is the composition of the two, and it does ! It is (pseudo code) for `g: B => C` and `f: A => B`, `g compose f` 
- That composition is associative: `(h compose g) compose f` is the same as `h compose (g compose f)`
...
We can now grasp how category theory can relate to pure functional programming !
...
A _functor_ `F` is a mapping between two categories means that it maps objects from category `A` to objects in the category `F(A)` (the category which `A` is mapped to by the _functor_ `F`) and morphisms from `A` to morphisms in `F(A)` while preserving their relations.

Since we always work with types and with function between types in Scala, a _functor_ in that context is a mapping from and to the __same category__, between `S` and `S` and that particular kind of _functor_ is called an __endofunctor__. 

Let's explore how `Option` behave (but we could have replaced `Option` by any _functor_ `F`):
...
Let's use our previously defined `def map[A, B](fa: F[A], func: A => B): F[B]`. 
...
We won't go into details but we could have shown how `Option` respect morphism composition and identity law.

I hope I made a bit clearer the parallel between theory and how it applies to FP by exploring what _functors_ are in category theory and how that translates to FP !
